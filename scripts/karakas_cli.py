...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export karakas.v1 from a *_full_pyjhora.json payload.")
...
def main() -> None:
    ...
    cfg = KarakasConfig.from_dict({"enabled": True})
    block = compute_karakas_from_payload(payload, cfg)
    output = json.dumps(block, indent=2, ensure_ascii=False)
    if args.out:
        Path(args.out).write_text(output + "\n", encoding="utf-8")
        if not args.no_print:
            print(f"Wrote karakas payload to {Path(args.out).resolve()}")
    else:
        print(output)
