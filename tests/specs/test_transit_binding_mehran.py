...
def test_transit_mehran_basic():
    payload = _prepare_transit_payload("references/in/mehran_birth.json")
    result = run_transit(payload)
    frame = result["frames"][0]

    assert result["meta"]["schema_version"] == "transit_spec_v1"
    assert frame["frame_id"] == "TRANSIT"
    assert frame["ascendant"]["sign_name"] in (
        "Pisces",
        "PISCES",
        "SCORPIO",
    )
    assert len(frame["planets"]) >= 5
